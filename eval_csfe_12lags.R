...
# CSFE h=1 ####
CSFE_h1_12lags = lapply(models_list, function(x){
...
# CSFE h=3 ####
CSFE_h3_12lags = lapply(models_list, function(x){
  cumsum((rw[,3]-yout[,1])^2)-((x[,3]-yout[,1])^2)
})%>% Reduce(f=cbind)
colnames(CSFE_h3_12lags) = model_files

matplot(1:nrow(CSFE_h3_12lags),  CSFE_h3_12lags, type="l", col=length(model_files), lty=1,
        xlab="Data", ylab="CSFE_h3_12lags")

# CSFE h=6 ####
CSFE_h6_12lags = lapply(models_list, function(x){
  cumsum((rw[,6]-yout[,1])^2)-((x[,6]-yout[,1])^2)
})%>% Reduce(f=cbind)
colnames(CSFE_h6_12lags) = model_files

matplot(1:nrow(CSFE_h6_12lags),  CSFE_h6_12lags, type="l", col=length(model_files), lty=1,
        xlab="Data", ylab="CSFE_h6_12lags")

# CSFE h=12 ####
CSFE_h12_12lags = lapply(models_list, function(x){
  cumsum((rw[,12]-yout[,1])^2)-((x[,12]-yout[,1])^2)
})%>% Reduce(f=cbind)
colnames(CSFE_h12_12lags) = model_files

matplot(1:nrow(CSFE_h12_12lags),  CSFE_h12_12lags, type="l", col=length(model_files), lty=1,
